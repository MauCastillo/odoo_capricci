...
def _rpc_count_modules(addr='http://127.0.0.1', port=8072, dbname='mycompany'):
    time.sleep(5)
...
class OdooDocker(object):
    def __init__(self):
        ...
        self.port = 8072  # TODO sle: reliable way to get a free port?
    ...
    def start(self, docker_image, build_dir, pub_dir):
        ...
        self.docker = pexpect.spawn(
            'docker run -v %s:/opt/release -p 127.0.0.1:%s:8072'
...
class KVM(object):
    ...
    def start(self):
        l="kvm -cpu core2duo -smp 2,sockets=2,cores=1,threads=1 -net nic,model=rtl8139 -net user,hostfwd=tcp:127.0.0.1:10022-:22,hostfwd=tcp:127.0.0.1:18072-:8072,hostfwd=tcp:127.0.0.1:15432-:5432 -m 1024 -drive".split(" ")
